...
if (user) return res.status(400).json({error : "That user already exists"})
...
const token = jsonwebtoken.sign({id: result.lastID}, process.env.JWT_SECRET, {expiresIn: "1h"})

res.cookie("token", token).json({message: "User created", id: result.lastID, token})
...
const token = jsonwebtoken.sign({id: user.id}, process.env.JWT_SECRET, {expiresIn: "1h"})

return res.cookie("token", token).json({message : "Succesful login", token})
    } catch (err) {
...
async function logout(req,res) {
    res.clearCookie("token").json({message: "Logout Succesfull"})
}
...
jsonwebtoken.verify(token, process.env.JWT_SECRET, (err,tok) => {
    if (err) return res.status(400).json({error: "Unauthorized"})
