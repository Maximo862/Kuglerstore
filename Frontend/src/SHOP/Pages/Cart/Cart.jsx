...
export function CartItem() {
  const { cart, pricecart, buttoncontinue ,Clearcart} = useContext(Cartcontext);
  ...
  return (
    <main className={"main_cart"}>
      ...
      <div className={"Precios mb-5"}>
        ...
        {cart.length > 0 && ( <button onClick={() => Clearcart()} className="clear-cart">Clear cart</button>)}
        <div className="cart-continue-box">
        {buttoncontinue && <Link className="cart-continue" to={"/Direction"}>Continue</Link>}
        </div>
      </div>
    </main>
  );
